<!-- BANNER-->
<p align="center">
  <img src="https://raw.githubusercontent.com/HoangCau-147/HoangCau-147/main/svg/banner.svg" alt="banner"/>
</p>

---

<div align="center" style="margin: 30px 0;">

**Catch me on** ✨

<a href="mailto:hvcau.work@gmail.com">
  <img src="https://img.shields.io/badge/email-0A66C2?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"/>
</a>&nbsp;&nbsp;
<a href="https://www.linkedin.com/in/hoangcau/">
  <img src="https://img.shields.io/badge/linkedin-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
</a>&nbsp;&nbsp;
<a href="https://www.facebook.com/hg.cau">
  <img src="https://img.shields.io/badge/facebook-0A66C2?style=for-the-badge&logo=facebook&logoColor=white" alt="Facebook"/>
</a>&nbsp;&nbsp;
<a href="https://github.com/HoangCau-147">
  <img src="https://img.shields.io/badge/github-0A66C2?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/>
</a>

</div>

### &nbsp;&nbsp;&nbsp;&nbsp; (ノ^∇^)  Glad to see you here! 
&nbsp;&nbsp;&nbsp;&nbsp; I'm **Hoàng Văn Cầu**, a passionate **Frontend Engineer** with a strong focus on **JavaScript** and **modern web frameworks**. With a solid foundation in software engineering and a mindset of continuous improvement, I strive to build clean, scalable, and high-performance web applications that provide excellent user experiences and solve real-world problems.

🚀 I am currently **seeking opportunities** where I can further enhance my skills, contribute effectively, and progress toward becoming a well-rounded Software Engineer.

---
## 🙋‍♂️ ABOUT ME
<table>
  <tr>
    <td width="60%" valign="top">
        🎓 Bachelor of Software Engineering, University of Science - VNUHCM with GPA: 3.37/4.00
        <br/> <br/>
        💼 Currently working at Frontend Engineer Position (ReactJS, Angular, NextJS)
        <br/> <br/>
        📚🎓🧠 Actively expanding my skills in Backend development to become a full-stack software engineer, aiming to deliver complete, end-to-end solutions.
        <br/> <br/>
        <div align="center">
            <a href="#">
                    <img src="https://raw.githubusercontent.com/HoangCau-147/HoangCau-147/main/svg/metrics.plugin.reactions.svg"/>
            </a>
        </div>
    </td>
    <td width="40%" align="center" valign="middle">
      <img src="https://raw.githubusercontent.com/HoangCau-147/HoangCau-147/main/banner.gif" width="320" style="border-radius:15px;"/>
    </td>
  </tr>
</table>

---

## </👨🏻‍💻> EXPERIENCE 
&nbsp;&nbsp;&nbsp;&nbsp; Throughout my academic journey and professional work, I have actively engaged with a variety of technologies, gaining practical hands-on experience and a solid understanding of modern development practices. 

&nbsp;&nbsp;&nbsp;&nbsp; The following technologies represent the areas in which I have applied my skills extensively:

- <img src="https://img.shields.io/badge/Algorithm & Programing-white?labelColor=black" alt="Algorithm & Programing">
    <img src="https://img.shields.io/badge/-OOP -- C%23 -- Python -- JavaScript -- C/C++-grey" alt="OOP, C#, Python, JavaScript, C/C++">

- <img src="https://img.shields.io/badge/Frontend-white?labelColor=black" alt="Frontend">
    <img src="https://img.shields.io/badge/-HTML -- CSS -- ReactJS -- Tailwind CSS -- Angular -- NextJS -- Redux -- SPA development-grey" alt="HTML, CSS, ReactJS, Tailwind CSS, Angular, NextJS, Redux, SPA development">

- <img src="https://img.shields.io/badge/Backend-white?labelColor=black" alt="Backend">
    <img src="https://img.shields.io/badge/-NodeJS -- NestJS -- ASP.NET Core -- WebAPI/Entity Framework -- Restfull API -- RabbitMQ-grey" alt="NodeJS, NestJS, ASP.NET Core, WebAPI/Entity Framework, Restfull API, RabbitMQ">

- <img src="https://img.shields.io/badge/Testing-white?labelColor=black" alt="Testing">
    <img src="https://img.shields.io/badge/-Jest -- Playwright -- Cucumber-grey" alt="Jest, Playwright, Cucumber">

- <img src="https://img.shields.io/badge/DevOps & Tools-white?labelColor=black" alt="DevOps & Tools">
    <img src="https://img.shields.io/badge/-Git/Github -- Docker -- Postman -- Elastichsearch -- Jira -- Power BI -- Figma-grey" alt="Git/Github, Docker, Postman, Elastichsearch, Jira, Power BI, Figma">

<table>
    <tr>
        <td width="50%" align="center" valign="middle">
            <a href="#">
                <img src="https://raw.githubusercontent.com/HoangCau-147/HoangCau-147/main/svg/metrics.plugin.isocalendar.svg"/>
            </a>
        </td>
        <td width="50%" align="center" valign="middle">
            <a href="#">
                <img src="https://raw.githubusercontent.com/HoangCau-147/HoangCau-147/main/svg/metrics.plugin.wakatime.svg"/>
            </a>
        </td>
    </tr>
</table>

<a href="#">
    <img src="https://raw.githubusercontent.com/HoangCau-147/HoangCau-147/main/svg/metrics.plugin.languages.indepth.svg"/>
</a>

## 📂 PROJECTS HIGHLIGHT
### Store Recommendation System
> A personalized e-commerce recommendation system analyzing user behavior and store metrics. 
- Role: Front-end & Team Leader 
- Technologies: ReactJS, Tailwind CSS, ASP.NET Web API, PhoBERT, Elasticsearch. 
- Algorithm: Neighborhood-based Collaborative Filtering, Content-based Filtering, Matrix Factorization.
- Achievements: Designed database, developed language model, implemented key pages, led team meetings, and produced thesis & project posters.

### Health Insurance Management System
> Web application to manage health insurance processes for customers and agents.
- Role: Front-end & Business Analyst
- Technologies: ReactJS, Tailwind CSS, ASP.NET Web API, EF Core. 
- Achievements: Designed database, performed business analysis, developed core pages, and collaborated on API integration.

### FinShark (Personal Finance Project)
> A financial web platform for learning and hands-on experience with full-stack development. 
- Role: Front-end | Backeend
- Technologies: ReactJS, TypeScript, .NET Core Web API, MS SQL Server. 
- Achievements: Built front-end components, implemented back-end CRUD APIs, applied DI & Repository pattern, and followed professional Git workflow.

---

<div align="right" style="text-transform: uppercase">
    <h3 style="margin-right: 100px;">Work Journey So Far</h3>
    <h3>Excited for What’s Next...</h3>
</div>

<p align="center">
  <img src="https://raw.githubusercontent.com/HoangCau-147/HoangCau-147/main/svg/bottom.svg" alt="bottom" width="100%"/>
</p>
